...
def EnterUrl(link,path):
    ...
    print("Seconds: "+endSeconds-startSeconds)
    print("Minutes: "+endSeconds-startSeconds/60)
    print("Hours: "+endSeconds-startSeconds/(60*60))
...
def santJiKhataInOrder(path):
    ...
    for i in range(len(theAngs)):
        if theAngs[i]==0:
            ...
            name="Default"+i+1+".mp3"
        else:
            down=theAngs[i]
            # name=f"Ang{i+1}.mp3"
            name="Ang"+i+1+".mp3"
        # urllib.request.urlretrieve(down,f"{path}\\{name}")
        urllib.request.urlretrieve(down,path+'\\'+name)
        print(folderPath+title)
